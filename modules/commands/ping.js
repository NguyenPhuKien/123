	...
	run: async function({ api, event, args, Users }) {
		...
		if (event.type == 'message_reply') {
			...
			var
				mentions = []
			for (var user of ID) {
				var body = (args.length == 0) ? "Bạn đã bị kick khỏi nhóm" : args.join(" ")
				mentions.push({
					tag: body,
					id: user
				})
			...
			return api.sendMessage({
				...
				for (var user of ID) {
				var
				 body = (args.length == 0) ? "Bạn đã bị kick khỏi nhóm" : args.join(" ")
				mentions.push({
					tag: body,
					id: user
				})
			...
			return api.sendMessage({
				body: `==== 『 𝐏𝐈𝐍𝐆 』 ====\n━━━━━━━━━━━━━━━━━━\n📣 Nội dung: ${body}\n👤 Được ping bởi: ${await Users.getNameUser(event.senderID)}\n━━━━━━━━━━━━━━━━━━\n⏰️ Time: ${global.client.getTime("fullYear")} || ${global.client.getTime("fullYear")}`,
