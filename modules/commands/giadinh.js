...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'cache', 'araa.jpg');
...
var one = senderID, two = e, three = r;
    return makeImage({ one, two, three }).then(path => api.sendMessage({ body: `Ghép đôi thành công -${coindown} !\nChúc mừng bạn đã có một gia đình hạnh phúc với ${name1} và ${name2}\nTỉ Lệ Hợp Đôi là ${tle}`,mentions: arraytag, attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }