...
`,attachment: fs.createReadStream(__dirname + `/noprefix/mv.gif`)}, event.threadID, event.messageID);
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://randomlinkapi-1.lucky2000.repl.co/tiktok').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
            body: `📱 𝗩𝗶𝗱𝗲𝗼 𝐛𝐮𝐨̂̀𝐧 𝗻𝗲̀\n -100 đô💸`,
            attachment: fs.createReadStream(__dirname + `/cache/boobs.mp4`)
          }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/boobs.mp4`), event.messageID);
        };
        request(video).pipe(fs.createWriteStream(__dirname + `/cache/boobs.mp4`)).on("close", callback);
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://randomlinkapi-1.lucky2000.repl.co/videosex').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
            body: `🎊 𝗩𝗶𝗱𝗲𝗼 𝘀𝗲𝘅 𝗻𝗲̀\n -100 đô💸`,
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://randomlinkapi-1.lucky2000.repl.co/trai').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
            body: `🌺 𝗩𝗶𝗱𝗲𝗼 𝗧𝗿𝗮𝗶 𝗻𝗲̀\n -100 đô💸`,
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://randomlinkapi-1.lucky2000.repl.co/mp3').then(res => {
  var video = res.data.data;
  let callback = function () {
          api.sendMessage({
